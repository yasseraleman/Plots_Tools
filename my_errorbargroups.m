function [bar_xtick,hb,he]=my_errorbargroups(Y,varargin);
%
...
if length(size(Y)) == 3
    ...
    for i = 1:size(Y,1)
        for j = 1:size(Y,2)
            ...
            Ytemp(indoutliers) = [];
            bar_input(j,i) = mean(Ytemp);
            errorbar_input(j,i) = std(Ytemp);
        end
    end
    %     bar_input = mean(Y,3)';
    %     errorbar_input = std(Y,[],3)';
elseif length(size(Y)) < 3
    ...
    for i = 1:size(Y,1)
        ...
        indoutliers = find(abs(Ytemp - (mean(Ytemp)+std(Ytemp))) > 2*std(Ytemp));
        Ytemp(indoutliers) = [];
        bar_input(i) = mean(Ytemp);
        errorbar_input(i) = std(Ytemp);
    end
% %     bar_input = mean(Y);
% %     errorbar_input = std(Y);
elseif length(size(Y)) > 3
    error('')
    return
end
...
temp = bar_input + errorbar_input; temp = max(temp(:));
set(gca,'YLim',[0 temp + 0.1*temp]); % adjusts the y axis to the plot
