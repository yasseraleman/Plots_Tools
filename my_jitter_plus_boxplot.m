function [jitter_xtick,hb,he]=my_jitter_plus_boxplot(Y,varargin)
%
...
% % optional_jitter_arguments = {'Markersize',4,'MarkerEdgeColor',[0 0 0],'LineStyle','none'};
% % boolface = 1;
% jitter_names = {'Frontal','Parietal','Temporal','Occipital'};
...
% %  %(Nstruct x Nmethods x Nsubjects;)
% % Y = squeeze(Var(1,:,:))';
% % Y(:,1) = Y(:,1)*10;

[N_subj,N_methods]=size(Y);
mean_vals = mean(Y);
std_vals = std(Y);
jitter_colors = Colors(1:N_methods,:); % get some distinguishable colors for the bars!
        ...
        if numel(varargin)>0 % optional input arguments are provided
            while ~isempty(varargin)
                ...
                switch varargin{1}
                    ...
                    case 'boolface'
                        boolface=varargin{2};
                    case 'jitter_separation'
                        jitter_separation=varargin{2};
                    otherwise
...
%Posi = [1:N_methods]-;
posi = repmat([1:jitter_separation:N_methods*jitter_separation],[size(Y,1) 1]) + (2*(rand(size(Y))-0.5)*0.2);
posiPoints = posi-jitter_width;
posiBox = posi+jitter_width;
%colordef white; hf = figure('numbertitle','off','name','New Figure','Color',[1 1 1], 'units','centimeters','Position',[20 10 10 10],'InvertHardcopy','off');
% plot the bars themselves
markers = strvcat('o','s','^','d','p','>','v');
for grp=1:N_methods
    if boolface
        h = plot(posiPoints(:,grp),Y(:,grp),'Marker',markers(grp),'MarkerFaceColor',jitter_colors(grp,:), optional_jitter_arguments{:});
    else
        h = plot(posiPoints(:,grp),Y(:,grp),'Marker',markers(grp),'MarkerEdgeColor',jitter_colors(grp,:), 'Markersize',4,'LineStyle','none');
    end
    hold on;
    hb = boxplot(gca,Y(:,grp),'positions',mean(posiBox(:,grp)),'widths',.3);

end
set(gca,'XTickLabel',{' '})

% cosmetic fine-tuning of the figure
set(gca,'XLim',[0 max(posiBox(:))+jitter_width]); % adjusts the x axis to the plot
gapval = std(Y(:));
set(gca,'YLim',[min(Y(:))-gapval  max(Y(:))+gapval ]); % adjusts the y axis to the plot
box off;

% set the x tick labels
set(gca,'XTick',mean(posi),'XTickLabel',jitter_names);
