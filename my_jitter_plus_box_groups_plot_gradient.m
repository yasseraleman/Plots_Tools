function varargout = my_jitter_plus_box_groups_plot_gradient(dataY,varargin)
%
% Syntax :
%  [FigID,outliersMat] = my_jitter_plus_box_groups_plot_gradient(dataY,varargin)
%
...
for grp=1:N_methods
    ...
    gradVariable = squeeze(gradVar(:,grp,:))';
    ...
    if pointsPlot
        if jitFill
            ...
            ind2rem = isnan(pgrad);
            pointColors = Val2colors(pgrad(:));
            pointColors(ind2rem,:) = [];
            
            if isempty(pointColors)
                hplot = plot(pXpos,pYpos,'Marker',markers(grp),'MarkerFaceColor',Colors(grp,:),'MarkerEdgeColor',[0 0 0],'LineStyle','none', 'Markersize',markSize*1.2);
                
            else
                pointColors = Val2colors(pgrad(:));
                hplot = scatter(pXpos(:),pYpos(:),markSize*7,pointColors,'filled','Marker',markers(grp),'MarkerEdgeColor',[0 0 0]);
            end
    ...
    if boxPlot
        ...
        for strs = 1:N_structs
            ...
            hpatch = patch(get(hbox(5),'XData'),get(hbox(5),'YData'),Colors(grp,:),'FaceAlpha',.1);
            %end
